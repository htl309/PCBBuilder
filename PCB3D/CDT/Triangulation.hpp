...
Triangulation<T, TNearPointLocator>::intersectedTriangle(
    ...
    throw Error("Could not find vertex triangle intersected by an edge.",
                    CDT_SOURCE_LOCATION);
}
    ...
    const VertInd startVertex) const
{
    ...
    if(loc == PtTriLocation::Outside) {
            throw Error("No triangle was found at position",
                        CDT_SOURCE_LOCATION);
    ...
    if(loc == PtTriLocation::OnVertex) {
        const VertInd iDupe = v1 == v   ? t.vertices[0]
            ...
            throw DuplicateVertexError(iV - m_nTargetVerts,
                                       iDupe - m_nTargetVerts,
                                       CDT_SOURCE_LOCATION);
